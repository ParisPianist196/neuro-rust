...
fn plot_synapses_signal(
    lifs: &mut FirstOrderLifCollection,
    synapses: &mut FirstOrderSynapsesCollection,
    ...
    let lif_outputs = lifs.step(inputs[0], T_STEP);
    let synapses_outputs = synapses.step(&lif_outputs, T_STEP)?;
    let trace = Scatter::new(inputs.clone(), synapses_outputs);
    plt.add_trace(trace);
    waveform.add_to_plot(&inputs, &mut plt, "Blah");
    // }
    plt.write_html("out.html");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut lifs =
        FirstOrderLifCollection::new(15, 0.02, 0.002, (-25., 100.), (-1., 1.), vec![-1, 1])?;
    let mut collection = FirstOrderSynapsesCollection::new(15, 0.05);
    plot_synapses_signal(&mut lifs, &mut collection)?;
    Ok(())
}
